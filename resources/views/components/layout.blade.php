...
<body class="h-screen flex bg-gray-100">
    <!-- Sidebar -->
    <aside class="w-1/5 bg-white p-5 h-full flex flex-col justify-between">
        <div>
            <div class="flex items-center gap-0 mb-9 space-x-2">
    ...
    <div class="flex-1 flex flex-col">
        <!-- Top Navbar -->
        <header class="w-full bg-white p-4 flex justify-end">
            <div class="flex flex-col text-right">
                <span class="font-medium font-sans text-base"><a href="/profile">{{ $email }}</a></span>
                <span class="font-regular font-sans text-base text-[#64748B]">{{ $name }}</span>
            </div>
        </header>

        <div class="bg-[#004BA4] w-full flex justify-between px-8 pt-6 pb-20 relative z-0">
            {{ $page }}
        </div>
        
        <!-- Main Content Slot -->
        <main class="flex-1 p-6 relative z-10 bg-white shadow-lg -mt-10 ml-7 mr-7 rounded-lg">
            {{ $slot }}
        </main>
    </div>
</body>
