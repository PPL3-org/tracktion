...
<body class="bg-gray-100">
    ...
    <div x-data="{ sidebarOpen: false }" class="flex h-screen transition-all duration-300">

        <!-- Sidebar -->
        <aside 
            :class="sidebarOpen ? 'w-1/5' : 'w-0'"
        ...
        <!-- Main Content -->
        <div :class="sidebarOpen ? 'ml-[20%]' : 'ml-0'" class="flex-1 transition-all duration-300">
            <!-- Navbar -->
            <header class="bg-white p-4 shadow-md flex justify-between items-center sticky top-0 z-50">
                <button @click="sidebarOpen = !sidebarOpen" class="text-[#004BA4] p-2 rounded-md bg-white hover:bg-[#003B82] group">
                    <svg width="24" height="24" class="group-hover:text-white" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="flex flex-col text-right">
                    <span class="font-medium font-sans text-sm"><a href="/profile">{{ $email }}</a></span>
                    <span class="font-regular font-sans text-sm text-[#64748B]">{{ $name }}</span>
                </div>
            </header>

            <div class="bg-[#004BA4] w-full flex justify-between px-4 pt-6 pb-20 relative z-0">
                {{ $page }}
            </div>
            
            <!-- Main Content Slot -->
            <main class="flex-1 relative z-10 bg-white shadow-lg -mt-10 rounded-lg h-full">
                {{ $slot }}
            </main>
        </div>
